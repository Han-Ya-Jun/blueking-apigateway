...
class MCPServerAppPermissionListApi(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # 由于 MCPServerAppPermission 表有 unique_together=("bk_app_code", "mcp_server") 约束，无需去重
        ...
        # 使用 Python 字典去重，保留每个 mcp_server_id 的最新记录
        approved_applies_queryset = MCPServerAppPermissionApply.objects.filter(
            bk_app_code=target_app_code,
            status__in=[MCPServerAppPermissionApplyStatusEnum.APPROVED.value],
        ).order_by("-applied_time")
        approved_applies = {}
        for obj in approved_applies_queryset:
            if obj.mcp_server_id not in approved_applies:
                approved_applies[obj.mcp_server_id] = obj

        # 构建 mcp_server_id -> handled_by 的映射
        handled_by_map = {mcp_server_id: obj.handled_by for mcp_server_id, obj in approved_applies.items()}
